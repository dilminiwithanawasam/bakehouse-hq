  ...
  const addBatchMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!qtyProduced || Number(qtyProduced) <= 0) {
        throw new Error(
          "Invalid quantity: Baking yield total volume parameters must be greater than 0.",
        );
      }
            ...
            <Table>
              ...
              <TableBody>
                {productsLoading && (
                  <TableRow>
                    <TableCell colSpan={4} className="text-center py-8 text-slate-400 font-medium">
                      Fetching active database registries...
                    </TableCell>
                  </TableRow>
                )}
                {!productsLoading && products.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={4} className="text-center py-8 text-slate-400 font-medium">
                      No product rows populated inside PostgreSQL database lines yet.
                    </TableCell>
                  </TableRow>
                )}
                {products.map((p: any) => (
              ...
              <TableHeader className="bg-slate-50">
                <TableRow>
                  ...
                  <TableHead className="font-bold text-slate-700">
                    Associated Bakery Product
                  </TableHead>
                  <TableHead className="text-right font-bold text-slate-700">
                    Available In-Stock Balance Quantity
                  </TableHead>
                  <TableHead className="font-bold text-slate-700 flex items-center">
                    <Calendar className="h-3 w-3 mr-1 text-slate-400" /> Shelf Life Expiry
                  </TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {batchesLoading && (
                  <TableRow>
                    <TableCell colSpan={4} className="text-center py-8 text-slate-400 font-medium">
                      Extracting batch timeline data...
                    </TableCell>
                  </TableRow>
                )}
                {!batchesLoading && batches.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={4} className="text-center py-8 text-slate-400 font-medium">
                      No live manufacturing baking batches logged in PostgreSQL database lines yet.
                    </TableCell>
                  </TableRow>
                )}
                {batches.map((b: any) => (
                  <TableRow key={b.id} className="hover:bg-slate-50/40">
                    ...
                    <TableCell className="font-bold text-slate-800">
                      {b.product_name || `Product Assignment Reference ID #${b.product}`}
                    </TableCell>
                    <TableCell className="text-right font-bold text-slate-700">
                      {b.current_quantity} / {b.quantity_produced} units
                    </TableCell>
                    <TableCell>
