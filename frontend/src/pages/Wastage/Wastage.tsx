...
import { currency, type WastageReason } from "@/services/mockData";
import { api, type Product, type Wastage } from "@/services/api";
              ...
              <form onSubmit={handleSubmit((v) => create.mutate(v))} className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    ...
                    {errors.qty && (
                      <p className="text-xs text-destructive font-bold">{errors.qty.message}</p>
                    )}
                  </div>
                  <div className="space-y-1.5">
                    <Label>Reason</Label>
              ...
              {wastages.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={6} className="text-center py-6 text-slate-400">
                    No logs compiled inside database files.
                  </TableCell>
                </TableRow>
              ) : (
                  ...
                  return (
                    <TableRow key={w.id} className="hover:bg-slate-50/40 transition-colors">
                      ...
                      <TableCell className="text-muted-foreground font-medium">
                        {w.recorded_by_name || w.recorded_by || "Workstation User"}
                      </TableCell>
                    </TableRow>
                  );
                })
