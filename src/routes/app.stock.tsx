...
function StockPage() {
  ...
  const healthy = products.filter(p => p.stock > p.minStock * 1.5).length;

  return (
    <>
      <PageHeader title="Stock counting" description="Reconcile physical counts with system inventory." />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Low / Critical" value={lowCount} icon={AlertTriangle} accent="destructive" />
        ...
        <Table>
          ...
          <TableBody>
            {filtered.map(p => {
              ...
              const s = statusOf(p.stock, p.minStock);
              return (
                <TableRow key={p.id}>
                  ...
                  <TableCell className={cn(
                    ...
                    diff < 0 ? "text-destructive" : diff > 0 ? "text-chart-4" : "text-muted-foreground"
                  )}>
