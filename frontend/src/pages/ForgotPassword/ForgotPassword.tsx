...
export function ForgotPasswordPage() {
  ...
  const onSubmit = async (data: { email: string }) => {
    try {
      ...
      toast.error(
        error.response?.data?.error?.message || "Could not bridge network request connections.",
      );
    }
  };
        ...
        {sent ? (
          <div className="text-center space-y-3 py-4">
            ...
            <p className="text-sm text-slate-500 font-medium leading-relaxed">
              We've dispatched a secure single-use cryptographic recovery link to your employee
              account email.
            </p>
            <Link
              to="/login"
              className="inline-flex items-center text-sm text-primary font-bold mt-3"
            >
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-4">
              <div className="space-y-1.5">
                ...
                <Input
                  id="email"
                  type="email"
                  placeholder="cashier@bakery.com"
              ...
              <div className="pt-1">
                <Link
                  to="/login"
                  className="inline-flex items-center text-sm text-muted-foreground hover:text-foreground font-bold"
                >
                  <ArrowLeft className="h-4 w-4 mr-1" /> Back to sign in
                </Link>
              </div>
