/**
 * Streamlined Application Layout Shell
 * Enforces strict Role-Based Access Control (RBAC) navigation filters and a11y compliance.
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex bg-background">
      ...
      {/* Sidebar - mobile responsive drawer layout */}
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          <aside className="relative w-64 bg-sidebar flex flex-col">
            <button 
              ...
              title="Close menu"
              className="absolute top-3 right-3 p-1 rounded-md hover:bg-sidebar-accent transition-colors" 
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-2 pl-2 pr-3 py-1.5 rounded-lg hover:bg-muted transition">
                  ...
                  <div className="hidden sm:flex flex-col items-start">
                    ...
                    <span className="text-[11px] text-muted-foreground font-semibold leading-tight">{ROLE_LABEL[user.role]}</span>
                  </div>
                </button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-56">
                ...
                <DropdownMenuItem onClick={() => router.navigate({ to: "/app/settings" })} className="font-semibold">
                  Settings
                </DropdownMenuItem>
                <DropdownMenuItem onClick={handleLogout} className="text-destructive focus:text-destructive font-bold">
  ...
  return (
    <>
      ...
      <div className="p-4 border-t">
        <div className="rounded-lg bg-sidebar-accent p-3 text-xs font-bold">
          <div className="flex items-center gap-2 mb-1">
            <Badge variant="secondary" className="bg-slate-900/10 text-slate-800 border-0 font-extrabold">
