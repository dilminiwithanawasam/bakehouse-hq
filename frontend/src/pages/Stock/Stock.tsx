...
export function StockPage() {
  ...
  const categories = useMemo(
    () => ["all", ...Array.from(new Set(products.map((p) => String(p.category))))],
    [products],
  );
  const filtered = products.filter(
    (p) =>
      (cat === "all" || String(p.category) === cat) &&
            ...
            {filtered.map((p) => {
              ...
              return (
                <TableRow key={p.id}>
                  ...
                  <TableCell className="text-muted-foreground">{String(p.category)}</TableCell>
