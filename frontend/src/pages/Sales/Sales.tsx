...
export function SalesPage() {
  ...
  const batches = productId
    ? allBatches.filter(
        (batch) =>
          String(batch.product) === productId && batch.is_active && batch.current_quantity > 0,
      )
    : [];
  ...
  const addItem = () => {
    ...
    if (existingIndex > -1) {
      ...
      if (testQty > currentBatch.current_quantity) {
        toast.error(
          `Cannot exceed stock limits. Combined cart total would be ${testQty}/${currentBatch.current_quantity}`,
        );
        return;
      }
