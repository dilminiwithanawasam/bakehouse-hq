...
function ExecutiveDashboard() {
  ...
  const wastageTrend = data?.wastage_breakdown?.map((w: any) => ({
    reason: w?.reason,
    count: w?.count ?? 0,
    loss: w?.total_loss ?? 0,
  })) ?? [];
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
        <StatCard label="Sales today"      value={currency(totalSalesToday)} delta="+12.4% vs yesterday" trend="up" icon={IndianRupee} />
        <StatCard label="Items sold"       value={totalItemsToday}           delta="+8.1%"                trend="up" icon={ShoppingBag} accent="amber" />
        <StatCard label="Wastage cost"     value={currency(wastageCost)}     delta="-4.2% vs avg"         trend="up" icon={Trash2} accent="destructive" />
        <StatCard label="Net revenue"      value={currency(totalSalesToday - wastageCost)} delta="+11.6%" trend="up" icon={TrendingUp} accent="sage" />
        <StatCard label="Low stock alerts" value={lowStockCount + outOfStockCount}  delta={`${lowStockCount + outOfStockCount} need restock`} trend="down" icon={AlertTriangle} accent="destructive" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-4">
        ...
        <ChartCard title="Category mix" description="Share of revenue">
          <ResponsiveContainer width="100%" height={280}>
            <PieChart>
              <Pie data={data?.period_comparison?.current_period?.categories ?? []} dataKey="total" nameKey="name" innerRadius={55} outerRadius={90} paddingAngle={3}>
                {(data?.period_comparison?.current_period?.categories ?? []).map((_: any, i: number) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}
              </Pie>
              <Tooltip contentStyle={tooltipStyle} />
        ...
        <ChartCard title="Wastage trend" description="Daily loss in INR">
          <ResponsiveContainer width="100%" height={240}>
            <LineChart data={wastageTrend}>
              <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" vertical={false} />
              <XAxis dataKey="reason" stroke="var(--muted-foreground)" fontSize={10} tickLine={false} axisLine={false} />
              <YAxis stroke="var(--muted-foreground)" fontSize={10} tickLine={false} axisLine={false} />
              <Tooltip contentStyle={tooltipStyle} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-4">
            ...
            {productSales.slice(0, 6).map((p) => (
              <div key={p.id} className="flex items-center justify-between py-2 border-b last:border-0">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">{p.qty} units · {currency(p.revenue)}</div>
                </div>
        ...
        <Card className="rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Badge variant="secondary" className="bg-destructive/10 text-destructive border-0">{lowStockCount + outOfStockCount}</Badge>
          </div>
          <div className="space-y-2">
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Sales today"   value={currency(totalSalesToday)} delta="+9% vs avg" trend="up" icon={IndianRupee} />
        ...
        <StatCard label="Pending counts" value={lowStockCount + outOfStockCount} delta={`${lowStockCount + outOfStockCount} categories low`} icon={Boxes} accent="destructive" />
      </div>
