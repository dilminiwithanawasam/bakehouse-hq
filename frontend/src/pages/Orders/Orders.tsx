...
export function OrdersPage() {
  ...
  const [outlet, setOutlet] = useState<string | null>(
    outlets[0]?.id ? String(outlets[0].id) : null,
  );
  ...
  const productsById = useMemo(() => {
    ...
    products.forEach((p: Product) => (map[String(p.id)] = p));
    return map;
  }, [products]);
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Select value={outlet || ""} onValueChange={(v) => setOutlet(v)}>
                    <SelectTrigger>
                      <SelectValue>
                        {outlets.find((o) => String(o.id) === outlet)?.name || "Select outlet"}
                      </SelectValue>
                    </SelectTrigger>
                    <SelectContent>
                      {outlets.map((o) => (
